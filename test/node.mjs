...
for (const module of modulesCoverage) {
  ...
  const status = supported === 0 ? colorize('bgRed', ' MOCK ') : colorize(unsupported ? 'bgYellow' : 'bgGreen', ` ${supported}/${all} `)
  const missingNames = unsupported > 3 ? module.unsupportedExports.slice(0, 3).join(', ') + `, and ${unsupported - 3} more...` : module.unsupportedExports.join(', ');
  const missing = missingNames ? colorize('gray', ` (missing: ${missingNames})`) : '';
  console.log(`${colorize(supported ? (unsupported ? 'yellow' : 'green') : 'red', `node:${module.name}`.padEnd(25))} ${status.padEnd(20)} ${missing}`);
}
