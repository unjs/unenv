...
export function pathToFileURL(
  ...
  if (windows && String.prototype.startsWith.call(filepath, "\\\\")) {
    ...
    outURL.hostname = punnycode.toASCII(hostname); // modified by unenv to use punycode
...
export function urlToHttpOptions(url: URL) {
  ...
  return {
    ...
    hostname:
      hostname && String.prototype.startsWith.call(hostname, "[")
        ? String.prototype.slice.call(hostname, 1, -1)
        : hostname,
    hash: url.hash,
    search: search,
    pathname: pathname,
...
function encodePathChars(filepath: string, options?: { windows?: boolean }) {
  const windows = options?.windows;
...
function getPathFromURLWin32(url: URL) {
  ...
  const letter = String.prototype.codePointAt.call(pathname, 1)! | 0x20;
...
function getPathFromURLPosix(url: URL) {
  ...
  for (let n = 0; n < pathname.length; n++) {
    if (pathname[n] === "%") {
      const third = String.prototype.codePointAt.call(pathname, n + 2)! | 0x20;
