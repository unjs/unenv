...
export const debuglog : typeof util.debuglog = (section, _cb) => {
  const fn = (msg: string, ...params: any[]) => { if (fn.enabled) { console.debug(`[${section}] ${msg}`, ...params); } };
  fn.enabled = true;
  return fn;
};
...
// @ts-ignore
export const inspect : typeof util.inspect = object => JSON.stringify(object, null, 2);

export const format: typeof util.format = (...args) => _format(...args);

export const formatWithOptions: typeof util.formatWithOptions = (_options, ...args) => _format(...args);
